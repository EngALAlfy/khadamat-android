import 'package:cached_network_image/cached_network_image.dart';
import 'package:flutter/material.dart';
import 'package:khadamat_app/config/API.dart';
import 'package:khadamat_app/mixins/FunctionsMixin.dart';
import 'package:khadamat_app/screens/ItemsScreen.dart';
import 'package:khadamat_app/screens/SubCategoriesScreen.dart';
import 'package:khadamat_app/screens/SubSubCategoriesScreen.dart';

class SubCategoryWidget extends StatelessWidget {
  final subcategory;

  const SubCategoryWidget({Key key, this.subcategory}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Container(
      child: ListTile(
        onTap: ()=> goTo(context, SubSubCategoriesScreen(subcategory: subcategory,)),
        leading: CachedNetworkImage(
          imageUrl: API.IMAGES_URL + "/" + subcategory.image,
          width: 50,
          height: 50,
        ),
        title: Text(subcategory.name),
        trailing: Wrap(
          direction: Axis.horizontal,
          crossAxisAlignment: WrapCrossAlignment.center,
          alignment: WrapAlignment.center,
          children: [
            Text("${subcategory.items_count}"),
            SizedBox(width: 5,),
            Icon(Icons.arrow_forward_ios)
          ],
        ),
      ),
    );
  }
}
